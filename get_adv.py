...
parser.add_argument('--ensemble_flag', default='1', type=str)
parser.add_argument('--arch', default='resnet152',help='source model for black-box attack evaluation',choices=model_names)
parser.add_argument('--snet_dir1', default='', help='the path of snet')
parser.add_argument('--snet_dir2', default='', help='the path of snet')
parser.add_argument('--snet_dir3', default='', help='the path of snet')
parser.add_argument('--snet_dir4', default='', help='the path of snet')
parser.add_argument('--snet_dir5', default='', help='the path of snet')
parser.add_argument('--snet_dir6', default='', help='the path of snet')
parser.add_argument('--snet_dir7', default='/', help='the path of snet')
parser.add_argument('--snet_dir8', default='', help='the path of snet')
####################
...
def main():
    ...
    if args.ensemble == True:
        ...
        model2 = nn.Sequential(Normalize(mean=net2.mean, std=net2.std), net2)
        model2 = model2.to(device)
        checkpoint = torch.load(args.snet_dir1)  # initial parameters of student model
        load_pretrained_model(model2, checkpoint['snet'])
        model2.eval()
        ####################################
        net3 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model3 = nn.Sequential(Normalize(mean=net3.mean, std=net3.std), net3)
        model3 = model3.to(device)
        checkpoint = torch.load(args.snet_dir2)  # initial parameters of student model
        load_pretrained_model(model3, checkpoint['snet'])
        model3.eval()
        ####################################

        net4 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model4 = nn.Sequential(Normalize(mean=net4.mean, std=net4.std), net4)
        model4 = model4.to(device)
        checkpoint = torch.load(args.snet_dir3)  # initial parameters of student model
        load_pretrained_model(model4, checkpoint['snet'])
        model4.eval()
        ####################################

        net5 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model5 = nn.Sequential(Normalize(mean=net5.mean, std=net5.std), net5)
        model5 = model5.to(device)
        checkpoint = torch.load(args.snet_dir4)  # initial parameters of student model
        load_pretrained_model(model5, checkpoint['snet'])
        model5.eval()
        ####################################

        net6 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model6 = nn.Sequential(Normalize(mean=net6.mean, std=net6.std), net6)
        model6 = model6.to(device)
        checkpoint = torch.load(args.snet_dir5)  # initial parameters of student model
        load_pretrained_model(model6, checkpoint['snet'])
        model6.eval()
        net7 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model7 = nn.Sequential(Normalize(mean=net7.mean, std=net7.std), net7)
        model7 = model7.to(device)
        checkpoint = torch.load(args.snet_dir6)  # initial parameters of student model
        load_pretrained_model(model7, checkpoint['snet'])
        model7.eval()
        net8 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model8 = nn.Sequential(Normalize(mean=net8.mean, std=net8.std), net8)
        model8 = model8.to(device)
        checkpoint = torch.load(args.snet_dir7)  # initial parameters of student model
        load_pretrained_model(model8, checkpoint['snet'])
        model8.eval()
        net9 = pretrainedmodels.__dict__[args.arch](num_classes=1000, pretrained='imagenet')
        model9 = nn.Sequential(Normalize(mean=net9.mean, std=net9.std), net9)
        model9 = model9.to(device)
        checkpoint = torch.load(args.snet_dir8)  # initial parameters of student model
        load_pretrained_model(model9, checkpoint['snet'])
        model9.eval()
        ####################################
        ensemble=Ensemble([model, model2,model3,model4,model5,model6,model7,model8,model9]).to(device)
    ...
    if args.gamma < 1.0:
        ...
        if args.arch in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152']:
            ...
            if args.ensemble==True:
                ...
                register_hook_for_resnet(model3, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model4, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model5, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model6, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model7, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model8, arch=args.arch, gamma=args.gamma)
                register_hook_for_resnet(model9, arch=args.arch, gamma=args.gamma)
