...
def forward_hook(gamma):
    # implement SGM through grad through ReLU
    def _forward_hook(module, grad_in, grad_out):
        if isinstance(module, nn.ReLU):
            return (grad_in,gamma * grad_out[0])
    return _forward_hook


def forward_hook_norm(module, grad_in, grad_out):
    # normalize the gradient to avoid gradient explosion or vanish
    std = torch.std(grad_out[0])
    return (grad_in,grad_out[0] / std)

def register_hook_for_resnet_forword(model, arch, gamma):
    ...
    forward_hook_sgm = forward_hook(gamma)

    for name, module in model.named_modules():
        if 'relu' in name and not '0.relu' in name:
            module.register_forward_hook(forward_hook_sgm)
        # e.g., 1.layer1.1, 1.layer4.2, ...
        # if len(name.split('.')) == 3:
        if len(name.split('.')) >= 2 and 'layer' in name.split('.')[-2]:
            module.register_forward_hook(forward_hook_norm)
